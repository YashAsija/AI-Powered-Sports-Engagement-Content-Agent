...
export const ViralityDashboard: React.FC<ViralityDashboardProps> = ({
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/90 dark:border-slate-800 p-4 sm:p-5 flex flex-col transition-colors shadow-2xs space-y-4">
      ...
      {/* 2. Active Card Deep-Dive */}
      <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200/80 dark:border-slate-750">
        ...
        {/* 4 Interaction Factor Badges with Tooltips */}
        ...
        <div className="mt-3 pt-2.5 border-t border-slate-200/60 dark:border-slate-700/60 space-y-1">
          ...
          {currentScore.keyDrivers.map((driver, i) => (
            <div key={i} className="text-[11px] text-slate-700 dark:text-slate-300 flex items-start gap-1 leading-snug">
              <span>{driver}</span>
            </div>
          ))}
        </div>
        ...
        <div className="mt-2 flex items-center gap-1.5 text-[10px] text-slate-500 dark:text-slate-400">
          ...
          <span>Optimal Window: <strong className="text-slate-700 dark:text-slate-300">{currentScore.bestTimeToPost}</strong></span>
        </div>
      </div>

      {/* 3. Card-by-Card Virality Ranking */}
      <div>
        <div className="flex items-center justify-between mb-2">
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between p-2 rounded-xl text-left transition-all text-xs border ${
                  isSelected
                    ? 'bg-orange-50 dark:bg-orange-950/30 border-orange-500/60 dark:border-orange-500/50 text-slate-900 dark:text-white shadow-2xs'
                    : 'bg-slate-50 hover:bg-slate-100 dark:bg-slate-800/40 dark:hover:bg-slate-800 border-slate-100 dark:border-slate-800 text-slate-700 dark:text-slate-300'
                }`}
              >
                <div className="flex items-center gap-2 min-w-0 pr-2">
                  ...
                  <div className="min-w-0">
                    ...
                    <span className="text-[10px] text-slate-600 dark:text-slate-300 uppercase font-medium">
